...
/**
 * write byte in PCA9685
 * 
 * @param reg register to write
 * @param value value to be written
 * @param handle I2C handle associated with device
 */
static void PCA9685_WriteByte(int handle, UBYTE reg, UBYTE value)
{
 ...
 * @param handle I2C handle associated with device
 */
static UBYTE PCA9685_ReadByte(int handle, UBYTE reg)
{
    return I2C_Read_Byte(handle, reg);
}
 ...
 * @param handle: I2C handle associated with device
 */
static void PCA9685_SetPWM(int handle, UBYTE channel, UWORD on, UWORD off)
{
 ...
 * @param handle: I2C handle associated with device
 */
void PCA9685_SetPWMFreq(int handle, UWORD freq)
{
    ...
    UBYTE oldmode = PCA9685_ReadByte(handle, MODE1);
    UBYTE newmode = (oldmode & 0x7F) | 0x10; // sleep

    PCA9685_WriteByte(handle, MODE1, newmode); // go to sleep
    PCA9685_WriteByte(handle, PRESCALE, prescale); // set the prescaler
    PCA9685_WriteByte(handle, MODE1, oldmode);
    DEV_Delay_ms(5);
    PCA9685_WriteByte(handle, MODE1, oldmode | 0x80);  //  This sets the MODE1 register to turn on auto increment.
}
 ...
 * @param handle: I2C handle associated with device
 */
void PCA9685_SetPwmDutyCycle(int handle, UBYTE channel, UWORD pulse)
{
 ...
 * @param handle: I2C handle associated with device
 */
void PCA9685_SetLevel(int handle, UBYTE channel, UWORD value)
{
    if (value == 1)
